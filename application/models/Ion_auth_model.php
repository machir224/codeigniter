...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, first_name, last_name, profile_pic, password, active, last_login')
						  ->where($this->identity_column, $identity)
						  ->limit(1)
						  ->order_by('id', 'desc')
						  ->get($this->tables['users']);
	...
	public function set_session($user)
	{
        ...
        if($user->profile_pic != '' && file_exists(FCPATH . 'uploads/images/profile/' . $user->profile_pic)){
